...
// Print a matrix of bits which, in this case, is an array of 64 bits ints
// printing each int with printBits
void printMatrix(uint16_t matrix[iBits]) {
	uint8_t i = 0;
...
// Copies the values between two arrays of size size
void copyArray16(uint16_t* out, uint16_t* in, size_t size) {
	uint8_t i;
...
int getXRandomBits(uint16_t* out, uint8_t startPosition, uint8_t numberBits, uint32_t randomBits[16], uint16_t* usedBits) {
...
int getXRandomBitsMatrix(uint16_t* out, uint8_t startPosition, uint8_t numberBits, uint32_t randomMatrix[8][16], uint8_t matrixPosition, uint16_t usedBits)
{
...
short testSingularity(uint16_t* matrix) {

	uint16_t ulAuxMatrix[iBits];
	uint16_t bitShiftAux = 0x8000;

	// Copying the matrix to an auxiliary matrix
	copyArray16(ulAuxMatrix, matrix, iBits);
...
void invertPermutationMatrix(uint16_t* permutMatrix) {
...
void invertUpperMatrix(uint16_t * upperMatrix) {
	...
	bitShiftAux = 1;//0x0000000000000001
...
void generateRandomMatrix()
{
	...
	for (i = 0; i < iBits - 1; i++) {
		...
		// Generate a 64 bits number where the bits < i are 0
		...
		bitShiftAux = ((uint16_t)1) << (iBits - (i + 1));
...
void generateInverseMatrix() {
	...
	// Now we'll need 120 * 2 + 6 = 4038 -> 7.88 = 8
	...
	// Generate and store all the random numbers we'll need
	// Now we'll need 120 * 2 + 6 = 246 -> 246/512 = 0... = 1
	...
	// usedBits = 246 - 0 * 512 = 246 //0 is the (1-1)
	uint16_t usedBits = 246 - (short)log2(iBits);
	uint8_t matrixPosition = 0; //?????????????????????????????????
	
	// 4th Step
	// Here we'll need log2(iBits) bits = 4 for iBits = 16
	//120 + 4 < 512
	uint16_t r = 0;
	...
	for (i = 0; i < iBits - 1; i++) {
		...
		bitShiftAux = ((uint16_t)1) << (iBits - 1);
...
// ------------------------------------------------------------------------------------
/*
// Return the first position in an long that has not yet been captured by any r in allR.
...
uint8_t findR(uint64_t v) {
	uint8_t i, x, y;
	uint64_t bitShiftAux = 0x8000000000000000;
...
// Creates an long composed ony by 0's and one 1 at the r'th position.
// Tested with all inputs from one matrix.
...
void generateMatrixNDana(uint64_t* A, uint64_t* T, uint8_t n, uint8_t aLine, uint8_t* allR, uint32_t* randomBits, uint16_t usedBits) {
	...
	if (n == 1) {
		...
		A[realPositionL] = A[realPositionL] ^ (((uint64_t)1) << (iBits - realPositionC - 1));

		T[realPositionC] = T[realPositionC] ^ (((uint64_t)1) << (iBits - realPositionC - 1));
...
void matrixMultiplication(uint64_t A[iBits], uint64_t T[iBits]) {
	uint8_t i, j, k;
	uint64_t bitShiftAuxi;
	uint64_t bitShiftAuxj;

	uint64_t tempU, tempC, tempA, tempT;

	// This represents the lines of A
	for (k = 0; k < iBits; k++) {
		tempU = 0;

		bitShiftAuxi = 0x8000000000000000;

		// This represents the columns of A and the lines of T
		for (i = 0; i < iBits; i++) {
			tempC = 0;

			bitShiftAuxj = 0x8000000000000000;
...
uint32_t test_gen32bits_nonsingularmatrix (void) //used_r is a list; r is an elemento supposed to be in used_r. n is the length of used_r
{
	...
	{ 
		//generateRandomMatrix();
		generateInverseMatrix();
		ones = x16bits_matrix_multiply_vector (ones1, ulMatrix);
