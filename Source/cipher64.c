...
uint64_t ulMatrix[iBits]; //The matrix to be generated 
uint64_t ulMask;  //The mask to be xored with some plaintext
...
short testSingularity(uint64_t* matrix) {
	...
	short sInvertible = 1;
	bitShiftAux = 0x8000000000000000;
...
void generateRandomMatrix()
{
	...
	for (i = 0; i < iBits - 1; i++) {
		...
		bitShiftAux = ((uint64_t)1) << (iBits - (i + 1));
...
void generateInverseMatrix() {
	...
	// Now we'll need 2016 * 2 + 6 = 4038 -> 7.88 = 8
	uint32_t randomMatrix[8][16];
	...
	// usedBits = 4038 - 7 * 512 = 454
	uint16_t usedBits = 454 - (short)log2(iBits);
	uint8_t matrixPosition = 7;
	...
	for (i = 0; i < iBits - 1; i++) {
		...
		bitShiftAux = ((uint64_t)1) << (iBits - 1);
	...
	// Take care of the mask
	getXRandomBits(&ulMask, 0, iBits, randomMatrix[7], &usedBits);
...
// where each matrix is a vector of 64 bit integer.
