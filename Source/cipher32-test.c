...
void copyArray(uint32_t* out, uint32_t* in, size_t size) {
	...
	if (size > iBits) { //if (size > iBits || size < 0) //unsigned
...
void generateInverseMatrix() {
	...
	uint32_t bitShiftAux = 0x80000000; //in 0b = 1000 0000 0000 0000 0000 0000 0000 0000.
...
uint32_t test_gen32bits_nonsingularmatrix (void) //used_r is a list; r is an elemento supposed to be in used_r. n is the length of used_r
{
	...
	ones = x32bits_matrix_multiply_vector (ones, ulMatrix);
...
short test_singularity (uint32_t * M1) //used_r is a list; r is an elemento supposed to be in used_r. n is the length of used_r
{
	//Create the Identity in which we will make the same opertions performed in M, in order to transfor the Identity into the Inverted matrix
	uint32_t * M = create_x32bits_matrix ();
	assign_x32bits_matrix (M, M1);//Assign M1 to M not to change the values in the original M1

	uint32_t Xor;//Only for simulation
	uint32_t * I = create_x32bits_identity();

	//CLEAN DOWNWARDS
	for (uint8_t i = 0; i<31; i++)//clean downwards for each M[i][i], i!=31
	{
		if (((M[i]>>(31-i)) & 1) == 0)// M[i][i] != 1 //Change with one row with one
		{
			for (uint8_t j = i+1; j<32; j++)//clean downwards, j!=0
				if ((M[j]>>(31-i)) & 1)
				{//IS IT NECESSARY O LOOK UPWARDS TO CHANGE ROWS?
					uint32_t aux = M[i];
					M[i] = M[j]; //Change rows
					M[j] = aux; //Change rows
					//Make the same to the Identity
					aux = I[i];
					I[i] = I[j]; //Change rows
					I[j] = aux; //Change rows
					break; //Changed!!!
		...
		//TEST SINGULARITY
		if (((M[i]>>(31-i)) & 1) == 0)//ONLY FOR TESTING IF TH MATRIX IS INVERTIBLE. CLEAN THIS LINE IN FUTURE
			return 0; //The matix is singular (not invertible)
		...
		for (uint8_t j = i+1; j<32; j++)//clean downwards, j!=0
		{
			...
			{
				M[j] = M[i] ^ M[j]; //Clean
				I[j] = I[i] ^ I[j]; //Make the same to the Identity
			}
			else
			{
				Xor = M[i] ^ M[j]; //Simulate cleaning
				Xor = I[i] ^ I[j]; //Make the same to the Identity
			}
		}
	}
	return 1;
...
{
	//gcc -std=c99 -Wall -O3 -o e cipher32-test.c -Iinclude -Llib -ltestu01 -lprobdist -lmylib -lm
	uint32_t seed1[16];//seed

 	//createSeed("password1", seed1, "nonce");//Convert String into array 
	for (uint32_t i = 0;  i<16; i++) //Creating the seed manually!
		seed1 [i] = i;

	initializeCipher (seed1);//Inicialize converted seed to global seed
