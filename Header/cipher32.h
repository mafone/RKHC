...
// Print a matrix of bits which, in this case, is an array of 32 bits int
// printing each int with "printBits"
void printMatrix(uint32_t u32matrix[iBits]);
...
// Receives a 16*32 bits seed (as an array) that is copied to a global variable used in salsa20
...
//Generate the inverse without having to generate the original matrix before (in order to reduce the runtime on decryption)
